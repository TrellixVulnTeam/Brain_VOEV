...
def message_parse(self, sender_socket, sender_name, message):
    ...
    # Case where user wants to join a room:
        ...
        if len(message.strip().split()) < 2:
            sender_socket.send(
                f"/join requires a #roomname argument.\nPlease enter: /join #roomname\n".encode())
        else:
    ...
    # Case where user wants to leave a room:
        ...
        if len(message.strip().split()) < 2:
            sender_socket.send(
                f"/leave requires a #roomname argument.\nPlease enter: /leave #roomname\n".encode())
        else:
            room_to_leave = message.strip().split()[1]
        if room_to_leave[0] != "#":
            sender_socket.send(
                f"/leave requires a #roomname argument to begin with '#'.\n".encode())
        else:
    ...
    # Pass the rest of the string along as the message
        ...
        if len(message) < 2:
            sender_socket.send(
                f"/send requires a #roomname(s) argument(s).\nPlease enter: /send #roomname(s)\n".encode())
        else:
            for word in message.split():
            	...
            	if rooms_to_send.count == 0:
                	...
                	convert_to_str = ' '.join([str(word)
                	for word in message_to_send])
                	convert_to_str = convert_to_str + '\n'

                	self.message_rooms(rooms_to_send, sender_socket,
                                   sender_name, convert_to_str)

    # list all members in a room
    elif message.split()[0] == "/members":
        if len(message.split()) != 2:
            sender_socket.send(
                f"/members requires a single #roomname argument.\nPlease enter: /members #roomname\n".encode())
